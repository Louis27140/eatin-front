...
const regexPassword = v => /^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$ %^&*-.]).{8,}$/.test(v) || 'Le mot de passe doit faire au moins 8 caractère et doit contenir une lettre majuscule, un chiffre et un caractère spécial'
